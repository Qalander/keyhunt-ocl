 ...
 * */
uint32_t Utils::hash_crc32(uint32_t crc32_in, const void* buf, size_t n) {
...
uint8_t* Utils::bin2hex(uint8_t * buf, const uint8_t * from, size_t n) {
	if (!buf)
		buf = (uint8_t*)malloc(n * 2 + 1);
	size_t a, b = 0;
	for (a = 0; a < n; a++) {
...
int Utils::hex2bin(uint8_t * buf, const uint8_t * from, size_t n) {
	if (!n)
		n = strlen((char*)from);
	if (n % 2 == 1) {
		return -1;       //Нечетное
...
void Utils::b58_encode_check(void* buf, size_t len, char* result) {
...
int Utils::b58_decode_check(const char* input, void* buf, size_t len) {
	int i, l, c;
...
void Utils::encode_privkey(const BIGNUM * bn, int addrtype, uint8_t * bin_result, uint8_t * wit_result) {
...
KeyInfo* Utils::get_key_info(const BIGNUM * private_key, PubType type) {

	KeyInfo* info = (KeyInfo*)calloc(1, sizeof(KeyInfo));
	...
	if (type == UNCOMPRESSED) {
		...
		info->publicu_bin[0] = 4;
		memcpy(&info->publicu_bin[1], info->public_x, 32);
		memcpy(&info->publicu_bin[33], info->public_y, 32);

		SHA256(info->publicu_bin, 65, info->public_sha256_bin);
	}
	else {

		info->publicc_bin[0] = (info->public_y[31] & 1) ? 3 : 2;
		memcpy(&info->publicc_bin[1], info->public_x, 32);

		SHA256(info->publicc_bin, 33, info->public_sha256_bin);
	}
	...
	if (type == UNCOMPRESSED) {
		for (j = 0; j < 65; j++) {
			info->publicu_hex[n++] = hex_asc_O(info->publicu_bin[j]);
			info->publicu_hex[n++] = hex_asc_0(info->publicu_bin[j]);
		}
		info->publicu_hex[n] = 0;
	}
	else {
		for (j = 0; j < 33; j++) {
			info->publicc_hex[n++] = hex_asc_O(info->publicc_bin[j]);
			info->publicc_hex[n++] = hex_asc_0(info->publicc_bin[j]);
		}
		info->publicc_hex[n] = 0;
	}
...
int Utils::set_pkey(const BIGNUM * bnpriv, EC_KEY * pkey) {
...
void Utils::hashrate_update(HashRate * hr, uint64_t value) {
